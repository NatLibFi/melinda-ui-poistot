...
export function handleFailedRouteParams(req, res, next) {
  ...
  const failedParams = [...checkPoistotRouteParams(routeParams)]
  ...
  //-----------------------------------------------------------------------------
  // Checks Poistotapp specific route parameters
  //  - possible Poistot route parameters are: 
  //  - all parameters are not present in every request
  //
  function checkPoistotRouteParams(routeParams) {
    return [];
  }
